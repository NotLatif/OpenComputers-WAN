...
local function routerHandleLAN(senderAddr, data)
    ...
    if body and body.title then
        ...
        elseif body.title == "DNS_POST" then
            if body.name == nil then
                local payload = {
                    title = "DNS_DENY",
                    reason = "No domain name provided"
                }
                routerCraftPacketAndSendToAddr(senderAddr, payload)
                return
            end
            local payload = {
                title = body.title,
                name = body.name,
                ip = routerData.public_ip
            }
            routerCraftPacketAndSendToAddr(routerData.isp.addr, payload)
        elseif body.title == "DNS_PUT" then
            if body.ip == nil then
                local payload = {
                    title = "DNS_DENY",
                    reason = "No domain name provided"
